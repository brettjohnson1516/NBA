...
Usage (PowerShell):
  python nba_sim_v2.py --rebuild --prior-seasons 2021 2022 2023
"""
...
def win_prob(W, adv, n_poss, margin, home_ball):
    # v2. NEAREST grid point, not searchsorted.
    #
    # np.searchsorted returns the first index whose grid value is >= adv, which
    # is a CEILING, not a rounding. ADV_GRID steps by 0.02, so every edge was
    # rounded up by 0.01 on average - always toward the home team, never away
    # from it. A typical edge for a 6-point favourite with 90 possessions left
    # per side is about 0.067, so the bias was roughly 15% of the quantity being
    # looked up, applied one-directionally to every state in every season.
    #
    # The trees partly absorbed it because it was present in training too, but
    # only on average: the bias in WP terms grows with possessions left and is
    # largest near a tied score, so no constant correction can remove it.
    #
    # Computed against both neighbours rather than by dividing by a step size,
    # so it stays correct if ADV_GRID is ever made non-uniform.
    idx = np.clip(np.searchsorted(ADV_GRID, adv), 1, len(ADV_GRID) - 1)
    lo_v = ADV_GRID[idx - 1]
    hi_v = ADV_GRID[idx]
    ai = np.where((adv - lo_v) < (hi_v - adv), idx - 1, idx)
