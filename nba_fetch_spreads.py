"""
nba_fetch_spreads.py
--------------------
Pulls Pinnacle closing POINT SPREADS and writes
<odds-dir>\\closing_spreads_pinnacle.parquet, keyed by espn_game_id so it joins
straight onto the feature tables alongside closing_lines_pinnacle.parquet and
closing_totals_pinnacle.parquet.

Design notes
------------
* Games and snapshot times come from your EXISTING closing_lines_pinnacle.parquet
  rather than being re-derived. The spread is therefore priced at the same
  moment as the moneyline close, and no separate schedule matching is needed.

* Default --snap-round is 0, meaning the exact snapshot_time recorded by
  nba_pinnacle_odds.py is requested. That timestamp is already a real snapshot
  on the API's grid, so the API serves that same snapshot back. Set
  --snap-round N to floor timestamps to an N-minute grid if you want more
  sharing between games at the cost of a possibly staler price.

* The historical endpoint returns a snapshot of EVERY event at one timestamp, so
  games sharing a snapshot minute cost one call between them.

* Every API response is cached to <odds-dir>\\spreads_cache\\. Re-runs read the
  cache and cost nothing, so an interrupted run resumes for free. This cache is
  separate from totals_cache because the markets parameter differs.

* Output is MERGED with any existing closing_spreads_pinnacle.parquet, not
  overwritten. Running for one season will not wipe the others.

* The spread is recorded home-referenced: home_spread_close is the handicap
  attached to the HOME outcome, so -6.5 means the home team is laying 6.5.

Requires an Odds API key in the ODDS_API_KEY environment variable:
  $env:ODDS_API_KEY = "<your-key>"

Usage (PowerShell):
  python nba_fetch_spreads.py --dry-run
  python nba_fetch_spreads.py --seasons 2026
  python nba_fetch_spreads.py --seasons 2021 2022 2023 2024 2025 2026
"""
...
def load_games(odds_dir: Path, seasons):
    """
    The game index is the moneyline file specifically. Globbing *closing*.parquet
    generally would happily pick up closing_totals_pinnacle.parquet or this
    script's own output, neither of which carries commence_time / snapshot_time.
    """
    cands = sorted(odds_dir.glob("*closing_lines*pinnacle*.parquet"))
    if not cands:
        cands = sorted(odds_dir.glob("*closing_lines*.parquet"))
    if not cands:
        raise FileNotFoundError(
            f"no *closing_lines*.parquet in {odds_dir}; run nba_pinnacle_odds.py "
            f"first, it produces the moneyline file this script indexes off")
    src = cands[0]
...
def fetch_snapshot(key, cache_dir: Path, api_key, bookmaker, regions,
    ...
    """One historical snapshot of all NBA spreads. Cached to disk."""
...
def extract(data, home_code, away_code, commence, tol_minutes):
    """Find this game in a snapshot and pull its spread and prices."""
        ...
        for bk in ev.get("bookmakers", []):
            for mk in bk.get("markets", []):
                ...
                for o in mk.get("outcomes", []):
                    code = TEAM_TO_CODE.get(o.get("name"))
                    if code == home_code:
                        home_pt, home_price = o.get("point"), o.get("price")
                    elif code == away_code:
                        away_pt, away_price = o.get("point"), o.get("price")
                # A spread market can quote only one side if the other outcome
                # is missing; derive the mirror rather than dropping the game.
                if home_pt is None and away_pt is not None:
                    home_pt = -float(away_pt)
                if away_pt is None and home_pt is not None:
                    away_pt = -float(home_pt)
                if home_pt is not None:
                    return {
                        "home_spread_close": float(home_pt),
                        "away_spread_close": float(away_pt),
...
def main():
    ...
    ap.add_argument("--snap-round", type=int, default=0,
                    help="floor snapshot times to N minutes so nearby games "
                         "share one API call; 0 uses the exact snapshot_time "
                         "from the moneyline file")
    ...
    cache_dir = odds_dir / "spreads_cache"
    cache_dir.mkdir(parents=True, exist_ok=True)
    out_path = odds_dir / "closing_spreads_pinnacle.parquet"
    ...
    rounding = (f"after rounding to {args.snap_round}min"
                if args.snap_round else "using exact snapshot times")
    print(f"  {len(games):,} games, {games['snap'].nunique():,} distinct "
          f"snapshots {rounding}")
    ...
    for g in games.itertuples(index=False):
        ...
        if rec is None:
            misses.append((g.espn_game_id, "event/spread not in snapshot"))
            continue
        rec.update({
    ...
    print(merged.groupby("season").agg(
        ...
        spread_min=("home_spread_close", "min"),
        spread_med=("home_spread_close", "median"),
        spread_max=("home_spread_close", "max")).to_string())
