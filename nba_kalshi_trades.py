#!/usr/bin/env python
"""
Stage 1 - pull every executed Kalshi trade on NBA game-winner markets
(KXNBAGAME) and write the backtest price tape.

This supersedes the first version, which had three defects:

  1. It read `t["count"]`. That field does not exist. The real field is
     `count_fp`, a STRING holding a FRACTIONAL contract count ("45.05"). Every
     size in the first pull was zero.
  2. It dropped `taker_book_side`. That field says whether the print hit the
     bid or lifted the ask, which is the most important column for deciding
     what price a fill was actually available at. Without it the backtest has
     to assume, and assuming costs about half a spread on every bet.
  3. It dropped `is_block_trade`. Block trades are negotiated off the book and
     were never available to a taker, so they must be excludable from the tape.

It also accumulated every row in one Python list before writing - roughly 40M
dicts. Trades are now written per-market as parquet shards and concatenated at
the end, so memory stays flat and an interrupted run resumes instead of
restarting.

Series and ticker format are confirmed, not guessed:
    KXNBAGAME-<yy><MON><dd><AWAY><HOME>-<YES>   e.g. KXNBAGAME-26JUN13NYKSAS-SAS
There is no start-time segment. (KXNBA is the championship futures series -
tickers like KXNBA-27-WAS - and is NOT game markets.)

Output columns
--------------
  game_date             date parsed from the ticker
  away_team, home_team  normalized 3-letter abbreviations
  ...
  timestamp_unix        Unix seconds as float; sub-second precision is kept
                        because prints arrive faster than 1/sec and the
                        backtest samples on a 5-second grid
...
Usage
-----
  python nba_kalshi_trades.py --api-key KEY --private-key <path-to-key.pem> \
      --start 2025-04-13 --outdir data\\kalshi
"""
...
log = logging.getLogger("nba_trades")
...
ABBR_ALIASES = {
    "BRK": "BKN", "BRO": "BKN", "NJN": "BKN", "NJ": "BKN", "BK": "BKN",
    "PHO": "PHX", "CHO": "CHA", "CHH": "CHA",
...
def fetch_markets(auth: KalshiAuth, session: requests.Session,
    ...
    """
    Every KXNBAGAME market across both tiers.

    Settled markets age out of /markets into /historical/markets. Pulling only
    one tier silently loses half the window, so both are fetched and merged on
    ticker.
    """
...
def _norm_abbr(code: str) -> Optional[str]:
    ...
    return c if c in VALID_NBA_ABBRS else None


def _split_teams(blob: str) -> Optional[tuple[str, str]]:
    """
    Split the concatenated away+home blob. Every split point is tried and both
    halves must resolve to real NBA abbreviations. More than one working split
    means the ticker is ambiguous, and None is returned rather than guessed.
    """
    hits = []
...
def parse_ticker(ticker: str) -> Optional[dict]:
    ...
    try:
        ...
        return None
    teams = _split_teams(m.group("teams"))
    if not teams:
        return None
    away, home = teams
    yes = _norm_abbr(m.group("yes"))
...
def fetch_trades_for_ticker(ticker: str, auth: KalshiAuth,
    ...
    tip-off and the first quarter, exactly what a live-betting backtest needs.
...
def main() -> int:
    ...
    ap.add_argument("--start", default="2025-04-13", help="YYYY-MM-DD, inclusive.")
    ...
    ap.add_argument("--shard-dir", type=Path, default=None,
                    help="Per-market parquet shards. Default <outdir>/shards. "
    ...
    log.info(f"Series {SERIES}, window {start} to {end}")

    shard_dir = args.shard_dir or (args.outdir / "shards")
    ...
    log.info(f"{len(kept):,} markets in window; {len(unparsed):,} tickers unparsed")
    if unparsed:
    ...
    out = args.outdir / "nba_kalshi_trades.parquet"
