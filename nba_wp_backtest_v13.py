...
Mechanics (as specified):
  ...
  * bet on either side whose net edge, after the taker fee, clears --min-edge.
    ...
    is unchanged.
  * size = edge-scaled, expanding, no look-ahead. Bets are walked in time order
    within each season and each one is z-scored against the edges of the bets
    BEFORE it, the size multiplier is 1 + --size-slope * z clipped to
    [--size-min, --size-max], normalised by the running average multiplier so
    the average size tracks --contracts. Nothing about the rest of the season is
    used to size a bet in November. The first --size-warmup bets of a season are
    sized flat, and the realised average lands near --contracts rather than
    exactly on it. --size-slope 0 = flat sizing.
...
def edge_scaled_size(edge, order, target_avg, slope, lo, hi, warmup, group=None):
    """
    Contracts per bet, scaled by the size of the edge, using ONLY the bets seen
    before it. No look-ahead: nothing about the final distribution of edges is
    used to size a bet placed in week one.

    Bets are walked in chronological order (restarting each season, if group is
    given). At each bet the mean and sd of edge come from the PRIOR bets only:

        z          = (edge - mean_prior) / sd_prior
        multiplier = clip(1 + slope * z, lo, hi)
        contracts  = round(target_avg * multiplier / mean_prior(multiplier))

    The normaliser is expanding too, so the running average size tracks
    target_avg without knowing where the season ends up. Because of that the
    realised average over the whole book lands NEAR target_avg, not exactly on
    it -- the exact-mean version needed the full-book distribution up front.

    The first `warmup` bets of each season have no distribution behind them and
    are sized flat at target_avg. Returns (contracts, z) in the original row
    order.
    """
    ...
    o = np.asarray(order)
    if o.dtype.kind == "M":
        o = o.astype("datetime64[ns]").astype("int64")
    o = np.asarray(o, dtype="float64")

    if group is None:
        g = np.zeros(n, dtype="int64")
    else:
        g = np.unique(np.asarray(group), return_inverse=True)[1].astype("int64")
    ...
    if n > 1:
        is_new[1:] = gs[1:] != gs[:-1]
    start = np.maximum.accumulate(np.where(is_new, pos, 0))
    kk = (pos - start).astype("float64")          # bets seen before this one
    ...
    ps = prior_sum(es)
    ps2 = prior_sum(es * es)
    denom = np.maximum(kk, 1.0)
    mean_p = ps / denom
    var_p = np.maximum(ps2 / denom - mean_p * mean_p, 0.0)
    sd_p = np.sqrt(var_p)

    live = (kk >= float(warmup)) & (sd_p > 1e-12) & (slope != 0.0)
    z = np.where(live, (es - mean_p) / np.where(sd_p > 0, sd_p, 1.0), 0.0)
    mult = np.where(live, np.clip(1.0 + slope * z, lo, hi), 1.0)

    pm = prior_sum(mult) / denom
    norm = np.where((kk > 0) & (pm > 1e-6), pm, 1.0)
    cs = np.maximum(1.0, np.rint(float(target_avg) * mult / norm))

    contracts = np.empty(n, dtype="float64")
    zout = np.empty(n, dtype="float64")
    contracts[idx] = cs
    zout[idx] = z
    return contracts, zout
...
def main():
    ...
    ap.add_argument("--size-warmup", type=int, default=100,
                    help="bets sized flat at the start of each season, before "
                         "there is a distribution of edges to compare against")
    ...
    # sized after the whole book is built so the home and away legs are walked as
    # one stream in time order; each bet only sees the edges that came before it
    n, zscore = edge_scaled_size(bets["edge"].to_numpy(),
                                 bets["grid_unix"].to_numpy("float64"),
                                 float(C), args.size_slope, args.size_min,
                                 args.size_max, args.size_warmup,
                                 bets["season"].to_numpy())
    bets["contracts"] = n
    bets["edge_z"] = zscore
    ...
    print(f"\nsizing: edge-scaled expanding (no look-ahead), slope {args.size_slope:g}, "
          f"multiplier clipped to [{args.size_min:.2f}, {args.size_max:.2f}], first "
          f"{args.size_warmup} bets of each season flat")
